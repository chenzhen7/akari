...
| pnpm Monorepo | `pnpm-workspace.yaml` | ✅ 完成 |
| 共享类型包 | `packages/shared-types/src/index.ts` | ✅ 完成 |
| Fastify 后端骨架 | `apps/server/src/index.ts` | ✅ 完成 |
| WebSocket 客户端 | `apps/web/src/hooks/useWebSocket.ts` | ✅ 完成 |
| 连接状态指示器 | `apps/web/src/components/layout/TopNav.tsx` | ✅ 完成 |
...
- [x] 创建 `packages/shared-types/`，将 `src/types/index.ts` 迁移至此，补充后端所需类型
...
- [x] 基础路由：`GET /health`、`GET /sessions`、`POST /sessions`
- [x] WebSocket 事件协议定义（参见下方协议表）
...
- [x] 创建 `apps/web/src/hooks/useWebSocket.ts`（连接管理、自动重连）
- [x] Session Store 从 Mock 数据改为接收 WebSocket 事件驱动更新
- [x] 连接状态指示器（TopNav 右侧）
