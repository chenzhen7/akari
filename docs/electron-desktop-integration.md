...
- 生产构建：不设置 `VITE_API_URL`，默认回到 `http://localhost:3001`
...
1. 后端
2. 前端 Vite（`VITE_API_URL=http://localhost:3001`，端口 `5173`）
3. Electron（注入 `NODE_ENV=development`）

通过 `fetch()` 轮询 `/health` 和前端根路径，确认就绪后再启动 Electron，退出时自动清理子进程。

采用 Node 脚本而不是 `concurrently + wait-on`，是因为 Windows 下 shell 引号、`&&`、`tcp` 等待逻辑不稳定，导致 `pnpm dev:desktop` 反复无法启动 Electron。

### 4.2 Electron dev 模式识别

`apps/desktop/src/main.ts` 中的 `isDev` 判断依赖 `NODE_ENV=development` 或 `--dev` 参数：

```ts
const isDev = process.env.NODE_ENV === 'development' || process.argv.includes('--dev')
```

`apps/desktop/package.json` 的 dev 脚本保持原样：

```json
"dev": "tsc && electron ."
```

`NODE_ENV=development` 由 `apps/desktop/scripts/dev.mjs` 在启动 Electron 时注入。未注入时 Electron 会误判为生产模式，进入 `startServer()` 分支，去找不存在的 `apps/server/dist/index.js`，窗口还没创建就失败。
...
| `apps/web/vite.config.ts`            | Vite proxy 配置                                     |
| `apps/web/src/lib/api.ts`            | `API_BASE` 常量                                     |
...
| 问题                               | 根因                                                                    | 修复                              |
| ---------------------------------- | ----------------------------------------------------------------------- | --------------------------------- |
| `pnpm dev:desktop` 不弹窗 / 不稳定 | Windows 下 `concurrently + wait-on` 的 shell 引号/`&&`/等待逻辑反复失效 | 改为 Node 编排脚本 `dev.mjs`      |
| Electron 误判为生产模式            | `pnpm --filter @akari/desktop dev` 未注入 `NODE_ENV=development`        | 由 `dev.mjs` 启动 Electron 时注入 |
