    ...
    for (const [workspaceId, manager] of this.sessionManagers) {
      promises.push(
        manager.dispose().then(() => {
          ...
          console.error(`[WorkspaceSessionRegistryService] dispose ${workspaceId} failed:`, err)
        })
      )
    }
  ...
  private scheduleDisposal(workspaceId: string): void {
    ...
    const timer = setTimeout(() => {
      ...
      manager.dispose().then(() => {
        ...
        console.error(`[WorkspaceSessionRegistryService] scheduled dispose ${workspaceId} failed:`, err)
      })
    }, this.ttlMs)
